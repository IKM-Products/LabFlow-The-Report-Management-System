  ...
  const fetchMyProfile = async (showLoading = true) => {
    ...
    try {
      ...
      setFormData({
        ...
        email: data.email || "",
        phone: data.phone || "",
        role: data.role_name || "ROLE_ADMIN",
      });
    } catch (err: any) {
      console.error("Error fetching profile:", err);
      setErrorMessage(
        err?.response?.data?.message || err.message || "Failed to load user profile details."
      );
    } finally {
            ...
            <div className="space-y-1">
              <div className="flex flex-wrap items-center gap-2.5">
                <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
                  {formData.firstName || "User"} {formData.lastName || "Profile"}
                </h1>
        ...
        <div className="flex items-center justify-between pb-6 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Edit the my admin account information in the system.
            </p>
          </div>
        </div>
