...
export default function TestCatalogForm({ defaultDeptId = "", onSuccess, onClose }: TestCatalogFormProps) {
  ...
  const onSubmit = async (data: TestCatalogFormData) => {
    ...
    setErrorsList([]);
    try {
      const res = await testCatalogService.createCatalog(data);
      if (res.success) {
        onSuccess();
        onClose();
      }
      ...
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl border border-slate-100 space-y-5">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-slate-800">Create Test Catalog</h2>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            ...
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Save Catalog"}
            </button>
          </div>
        </form>
      </div>
