...
export default function ReferenceRangeForm({
  ...
  const onSubmit = async (data: ReferenceRangeFormData) => {
    ...
    setErrorsList([]);
    try {
      const res = await referenceRangeService.createReference({
        ...data,
      ...
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl border border-slate-100 space-y-5 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-slate-800">Create Reference Range</h2>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex gap-2 pt-2">
            ...
            >
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Save Reference"}
            </button>
          </div>
        </form>
      </div>
