...
export const TestCatalogSchema = z.object({
  ...
  test_price: z.number({ message: "Price is required" }).min(0, "Price must be positive"),
  turnaround_time: z.number({ message: "Turnaround time is required" }).min(0, "Turnaround time must be positive"),
});
